...
begin
  ...
  if requester_role = 'project_manager' and old.created_by_role = 'admin' then
...
using (
  exists (
    ...
    where tasks.id = task_labels.task_id
      and get_workspace_role(tasks.workspace_id) in ('admin', 'project_manager', 'member')
  )
)
with check (
  exists (
